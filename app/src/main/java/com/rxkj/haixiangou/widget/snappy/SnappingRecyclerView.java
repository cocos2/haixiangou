...
public class SnappingRecyclerView extends RecyclerView {
  ...
  @Override public boolean onTouchEvent(MotionEvent e) {
    ...
    if (lm instanceof ISnappyLayoutManager && (e.getAction() == MotionEvent.ACTION_UP
        || e.getAction() == MotionEvent.ACTION_CANCEL) && getScrollState() == SCROLL_STATE_IDLE) {
  ...
  @Override public boolean fling(int velocityX, int velocityY) {
    ...
    if (lm instanceof ISnappyLayoutManager) {
      super.smoothScrollToPosition(
          ((ISnappyLayoutManager) getLayoutManager()).getPositionForVelocity(velocityX, velocityY));
      return true;
    }
